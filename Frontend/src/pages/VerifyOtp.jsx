...
export default function VerifyOtp() {
  ...
  async function handleResend() {
    ...
    setResending(true);
    try {
      const userId = localStorage.getItem('lockify-userId');
      ...
      if (!res.ok) { setOtpError(data.error || 'Could not resend code.'); return; }
 ...
 async function handleVerify() {
    ...
    // For forgot mode — just save OTP and show reset form (no API call yet)
    if (mode === 'forgot') {
      setVerifiedOtp(code);
      setVerified(true);
      return;
    ...
    try {
      ...
      localStorage.setItem('lockify-token', data.token);
      localStorage.setItem('lockify-user',  JSON.stringify(data.user));
  ...
  async function handleResetPassword(e) {
    ...
    try {
      ...
      if (!res.ok) {
        setPassErrors({ general: data.error || 'Failed to reset password.' });
        return;
      }

      // Clear stored userId and go to login
      localStorage.removeItem('lockify-userId');
      navigate('/login');

    } catch {
      setPassErrors({ general: 'Could not connect to server.' });
    } finally {
      setLoading(false);
    }
  }
