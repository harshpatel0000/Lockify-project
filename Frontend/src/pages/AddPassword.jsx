...
export default function AddPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ general: data.error || 'Failed to save. Please try again.' });
        return;
      }
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-wrap">
                      ...
                      <div style={{ position:'absolute', right:'8px', display:'flex', alignItems:'center', gap:'4px' }}>
                        <button type="button" style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', display:'flex', alignItems:'center', padding:'4px' }} onClick={handleCopy} title="Copy">
                          {copied ? <CheckCircleIcon size={14} style={{ color:'#34d399' }} /> : <CopyIcon size={14} />}
                        </button>
                        <button type="button" style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', display:'flex', alignItems:'center', padding:'4px' }} onClick={() => setShowPass(s => !s)} title="Toggle visibility">
