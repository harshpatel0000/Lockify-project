...
const STATS = [
  { label:'Total Passwords', value:'48',  Icon:KeyIcon,         color:'stat-blue',    sub:'+3 this week'     },
  { label:'Security Score',  value:'87%', Icon:ShieldIcon,      color:'stat-green',   sub:'4 need attention' },
  { label:'Weak Passwords',  value:'5',   Icon:AlertCircleIcon, color:'stat-amber',   sub:'Update these'     },
  { label:'Breach Alerts',   value:'0',   Icon:CheckCircleIcon, color:'stat-success', sub:'All clear'        },
];
      ...
      {[['Username', item.username, 'user'], ['Password', item.password, 'pw']].map(([label, val, key]) => (
        <div key={key}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:6, background:'var(--c-bg2)', border:'1px solid var(--c-border)', borderRadius:7, padding:'5px 8px' }}>
            <span style={{ fontSize:'0.76rem', color:'var(--c-text2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1, fontFamily: key==='pw'?'monospace':'inherit', letterSpacing: key==='pw'?'0.1em':'normal' }}>{key==='pw'?'••••••••••':val}</span>
...
export default function Dashboard() {
  ...
  const user  = JSON.parse(localStorage.getItem('lockify-user') || '{}');
  ...
  async function fetchVault() {
    try {
      ...
      if (res.ok) {
        // Map API items to match UI shape
        setVault(data.items.map(item => ({
          ...item,
          id:    item._id,
          Icon:  GlobeIcon,
          color: ['vi-green','vi-blue','vi-amber','vi-red','vi-purple'][Math.floor(Math.random()*5)],
        })));
  ...
  async function toggleFav(id) {
    try {
      const res = await fetch(`/api/vault/${id}/favorite`, {
        method:  'PATCH',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok) {
        setVault(p => p.map(it => it.id === id ? { ...it, favorite: data.favorite } : it));
      }
    } catch (err) {
  ...
  const statsCards = stats ? [
    { label:'Total Passwords', value: String(stats.total),          Icon:KeyIcon,         color:'stat-blue',    sub:`${stats.favorites} favorites`      },
    { label:'Security Score',  value: `${stats.score}%`,            Icon:ShieldIcon,      color:'stat-green',   sub:`${stats.weak} need attention`      },
    { label:'Weak Passwords',  value: String(stats.weak),           Icon:AlertCircleIcon, color:'stat-amber',   sub:'Update these'                      },
    { label:'Strong Passwords',value: String(stats.strong),         Icon:CheckCircleIcon, color:'stat-success', sub:'Keep it up!'                       },
  ] : [];
      ...
      <aside className={`db-sidebar ${sideOpen?'db-sidebar-open':''}`}>
        ...
        <div className="db-sidebar-bottom">
          <div className="db-user">
            <div className="db-avatar">{(user.name||'U').slice(0,2).toUpperCase()}</div>
        ...
        <div className="db-content">
          <div className="db-page-head">
            <div>
              ...
              <p style={{fontSize:'0.82rem',color:'var(--c-text3)',marginTop:2}}>Welcome back, {user.name?.split(' ')[0] || 'there'}. Your vault is secure.</p>
            </div>
          ...
          <div className="db-stats">
            {statsCards.map(s=>(
              <div key={s.label} className={`db-stat-card ${s.color}`}>
                <div className="db-stat-icon"><s.Icon size={18}/></div>
          ...
          {favorites.length > 0 && (
            <section style={{marginBottom:28}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
                <div style={{display:'flex',alignItems:'center',gap:8,fontWeight:700,fontSize:'0.95rem',color:'#dde3ff'}}>
                  <span style={{color:'#fbbf24'}}><IcoStar filled={true}/></span>
                  Quick Access
                  <span style={{minWidth:20,height:20,padding:'0 6px',background:'rgba(251,191,36,0.15)',border:'1px solid rgba(251,191,36,0.3)',borderRadius:999,fontSize:'0.7rem',fontWeight:700,color:'#fbbf24',display:'inline-flex',alignItems:'center',justifyContent:'center'}}>{favorites.length}</span>
                </div>
                <Link to="/passwords" style={{fontSize:'0.8rem',fontWeight:600,color:'var(--c-primary)',textDecoration:'none'}}>View all →</Link>
              </div>
          ...
          {loading && (
            <div className="db-empty">
              <h3 className="db-empty-title">Loading your vault…</h3>
            </div>
          )}
          ...
          {!loading && filtered.length===0 && (
            <div className="db-empty">
              ...
              <h3 className="db-empty-title">{vault.length === 0 ? 'Your vault is empty. Add your first password!' : 'No results found'}</h3>
            </div>
          )}
        </div>
      </main>
